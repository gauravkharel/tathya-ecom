...
import { GetProductsQueryParams } from '@/api/products';
...
const NestedMenu = () => {
    ...
    const handleChange = (categoryName: string) => {
        //initally, redirect to the product list route and later only filter.
                    ...
                    <NavigationMenu>
                        <NavigationMenuItem>
                            <NavigationMenuTrigger asChild className='p-1  font-medium'>
                                <Link href={'/products'} onClick={() => handleChange(category.name)} className="cursor-pointer flex hover:text-gray-600 transition-colors">
                                    {category.name}
                                </Link>
